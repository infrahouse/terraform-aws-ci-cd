...
TEST_ROLE ?= arn:aws:iam::303467602807:role/ci-cd-tester
TEST_SELECTOR ?= aws6

.PHONY: help
help: install-hooks  ## Show this help
...
.PHONY: test-keep
test-keep:  ## Run tests and keep resources
	pytest -xvvs \
...
.PHONY: test-clean
test-clean:  ## Run tests and destroy resources
	pytest -xvvs \
		--aws-region=${TEST_REGION} \
		--test-role-arn=${TEST_ROLE} \
		-k $(TEST_SELECTOR) \
...
clean:  ## Remove various artifacts
	...
	find . -name '.terraform.lock.hcl' -path '*/test*' -exec rm -f {} +
